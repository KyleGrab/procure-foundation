...
def test_locations_valid_org_and_cross_tenant_and_empty_string_residue():
    with psycopg.connect(_admin_dsn(), autocommit=True) as admin_conn, admin_conn.cursor() as cur:
        cur.execute(
    ...
    try:
        with psycopg.connect(_app_dsn(), autocommit=True) as conn, conn.cursor() as cur:
